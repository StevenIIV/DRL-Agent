...
class DuelingDQNAgent():
    ...
    def __init__(self, state_size, action_size, seed, obs_mode, gamma=0.99, lr=5e-4, update_fre=4):
        ...
        # Q-Network
        if obs_mode == 'info':
            self.qnetwork_local = Dueling_NN(state_size, action_size, seed).to(device)
            self.qnetwork_target = Dueling_NN(state_size, action_size, seed).to(device)
        elif obs_mode == 'img':
            self.qnetwork_local = ConvDueling_NN(state_size, action_size, seed).to(device)
            self.qnetwork_target = ConvDueling_NN(state_size, action_size, seed).to(device)
